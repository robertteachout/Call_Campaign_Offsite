...
def main(projects, location):
    ...
    group = pd.DataFrame()
    total = pd.DataFrame(
        {"total": ["total_inventory", "total_calls", "total_no_calls"]}
    ).set_index("total")
    ...
    oc = start[start.Project_Type.isin(projects)][
        ["Project_Type", "PhoneNumber", "OutreachID", "Last_Call", "Skill"]
    ]
    ...
    nic_data = nic_data.rename(
        columns={"Skill_Name": "Skill", "Contact_Name": "PhoneNumber"}
    )
    ...
    disp = (
        df_drop.groupby("Disp_Name")["OutreachID"]
        .count()
        .to_frame()
        .rename(columns={"OutreachID": f"{yesterday}"})
        .sort_values(by=f"{yesterday}", ascending=False)
    )
    top = 10
