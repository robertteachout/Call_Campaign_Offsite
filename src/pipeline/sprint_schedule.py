...
def static_schedule(B10):
    dt = pd.DataFrame({"startdate": B10, "Number": range(10)})
...
def daily_maping(df, assignment, tomorrow_str):
    ...
    assignment["Daily_Groups"] = pd.to_datetime(
        assignment["Daily_Groups"], format="%Y-%m-%d"
    )
...
def map_priotiy(df, Day, names):
    ...
    df["Daily_Priority"] = (
        df["Daily_Groups"].map(Daily_sort).fillna(Sprint_schedule[-1])
    )
    return df
...
def Assign_Map(df, B10, num1, num2):
    ...
    def assign_skill(sk, j, BusDay):
        ...
        try:
            Daily_Priority = Daily_Priority.append(
                Daily_Priority.iloc[[-1] * add_back]
            ).reset_index(drop=True)
        except IndexError:
            pass
